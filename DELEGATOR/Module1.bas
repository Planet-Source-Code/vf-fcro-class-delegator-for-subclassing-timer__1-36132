...
Declare Function SetTimer& Lib "user32" (ByVal hwnd As Long, ByVal nIDEvent As Long, ByVal uElapse As Long, ByVal lpTimerFunc As Long)
...
CalculateSpaceForDelegation = 27 + NumberOfParameters * 3
End Function

Public Function DelegateFunction(ByVal CallingADR As Long, Obj As Object, ByVal MethodAddress As Long, ByVal NumberOfParameters As Byte, ByRef TempStore As Long) As Boolean
On Error GoTo NotSuccess
Dim TmpA As Long
TmpA = CallingADR
CopyMemory ByVal CallingADR, &H68EC8B55, 4
CallingADR = CallingADR + 4
CopyMemory ByVal CallingADR, VarPtr(TempStore), 4
CallingADR = CallingADR + 4
...
'PUSH EBP
'MOV EBP,ESP
'PUSH ADDRESSOF RETURN VARIABLE
...
'MOV EAX,DWORD PTR [ADDRESSOF RETURN VARIABLE]
'LEAVE
